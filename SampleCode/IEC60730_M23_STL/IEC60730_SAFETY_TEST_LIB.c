...
void CLASSB_SYSTEM_RESET_T()
{
	/* Unlock protected registers */
    do
    {
        SYS_NS->REGLCTL = 0x59UL;
        SYS_NS->REGLCTL = 0x16UL;
        SYS_NS->REGLCTL = 0x88UL;
    }
    while(SYS_NS->REGLCTL == 0UL);
    /* System Reset Implementations*/
    SYS_NS->IPRST0 |= SYS_IPRST0_CHIPRST_Msk;
...
uint8_t CLASSB_SAFE_STATE(uint8_t u8TestResult)
{
    ...
    if(SAFE_STATE == 0)
        CLASSB_IDLE_STATE{g_APP_pfnWatchDogReset();}
    else if (SAFE_STATE == 1)
        CLASSB_RESET_STATE;
    else if (SAFE_STATE == 2)
		CLASSB_RESET_STATE_T;
    return 0;
}
